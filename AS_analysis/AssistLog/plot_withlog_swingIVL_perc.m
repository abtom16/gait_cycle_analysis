...
for i=1:length(filepaths)
    ...
    for j=1:length(paretic_contact_end_frame_time)-1
        ...
        plot(time(idx_angle), paretic_angle_data(idx_angle),'DisplayName','Paretic Side Data', 'Color', [0, 0, 1], 'LineWidth',2); hold on;
        ...
        xlabel('Gait Cycle [%]', 'FontSize', 16, 'Units', 'data', 'Position', [mean(xlim()), xlabel_y_pos]);
        ...
        %% add text for assist timing percentage (%) -- Hundles cases where multiple assist start times exist 
        for k=1:length(assist_start_time)
            if current_start_time <= assist_start_time(k) && assist_start_time(k) <= current_end_time
                assist_start_perc = (assist_start_time(k) - current_start_time) / current_gaitcycle_time * 100;
                assist_start_str = sprintf('%.2f%%', assist_start_perc);
        ...
        for k=1:length(assist_end_time)
            if current_start_time <= assist_end_time(k) && assist_end_time(k) <= current_end_time
                ...
                assist_end_perc = (assist_end_time(k) - current_start_time) / current_gaitcycle_time * 100;
                assist_end_str = sprintf('End: %.2f%%', assist_end_perc);
        ...
        stance_start_perc = (stance_start_time - current_start_time) / current_gaitcycle_time * 100;
        ...
        yyaxis left;
        text(current_start_time, text_y_pos,'0', ...
             'HorizontalAlignment', 'center','VerticalAlignment', 'top','FontSize', 14);   
        text(stance_start_time, text_y_pos,sprintf('%.2f', stance_start_perc), ...
             'HorizontalAlignment', 'center','VerticalAlignment', 'top','FontSize', 14);
        text(current_end_time, text_y_pos,'100', ...
             'HorizontalAlignment', 'center', 'VerticalAlignment', 'top','FontSize', 14);
        if ~isempty(unaffected_footcontact_time)
            ...
            text(unaffected_footcontact_time, text_y_pos, sprintf('%.2%%', unaffected_footcontact_perc), ...
                'HorizontalAlignment', 'center', 'VerticalAlignment', 'top', 'FontSize', 14);
            end
        end
        %% --
        ...
        text(joint_event_time, text_joint_y, [joint_event_str, sprintf('%.2f%%', joint_event_perc)], ...
        ...
        filename= [filenames{i}, filename_joint,sprintf('swingIVL_step_%02d.jpeg', j)]; 
