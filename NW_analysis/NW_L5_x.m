...
save_folder = 'C:\abe_backup\backup\01_修士\06_Xsens_analysis\07_clinical-analysis\L5_x';
...
for p_idx = 1:2:length(nw_paths)
    ...
    for j = 1:length(current_nw_contact_frame)-1
        current_cycle_foot_x = current_nw_data.(foot_side).x(current_nw_contact_frame(j));
        current_cycle_L5_x = current_nw_data.L5.x( ...
            current_nw_contact_frame(j):current_nw_contact_frame(j+1)); 
        
        L5_x = current_cycle_L5_x - current_cycle_foot_x;
        L5_x = L5_x * 100;
        original_x = linspace(0, 1, length(L5_x));
        uniform_x_temp = linspace(0, 1, UNIFORM_LENGTH);
        resampled_cycle = interp1(original_x, L5_x, uniform_x_temp);
    ...
    abs_nw_mean_resampled = abs(nw_mean_resampled);
    [min_val_stance, min_idx_stance] = min(abs_nw_mean_resampled(1:50));
    
    %  -- plot
    fig = figure('Visible','off');
    [cfg1, ~] = plot_config('L5 Forward');  %
    ...
    text(min_idx_stance/100, min_val_stance-10, sprintf('Pelvis Over Foot Timing %d%%', min_idx_stance), ...
