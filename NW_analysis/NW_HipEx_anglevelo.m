...
save_folder = 'C:\abe_backup\backup\01_修士\06_Xsens_analysis\07_clinical-analysis\HipEx_AngularVelo';
...
filename_target_joint = '_HipEx_anglevelo.png';
...
for p_idx = 1:length(nw_paths)
    ...
    for j = 1:length(current_nw_contact_frame)-1
        ...
        angular_velo = zeros(length(current_cycle), 1);
        angular_velo(1) = (current_cycle(2) - current_cycle(1)) / dt;
        for k = 2:length(current_cycle) - 1
            angular_velo(k) = (current_cycle(k+1) - current_cycle(k-1)) / (2 * dt);
        end
        angular_velo(end) = (current_cycle(end) - current_cycle(end-1)) / dt;
        %% --------------------------------------
    ...
    nw_mean_resampled = mean(all_nw_resampled_angle_velos, 2);
    nw_std_resampled = std(all_nw_resampled_angle_velos, 1, 2);
    ...
    [cfg1, ~] = plot_config('Hip Flexion Angular Velocity');  
